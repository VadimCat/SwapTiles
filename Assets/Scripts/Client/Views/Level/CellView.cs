...
namespace Client.Views
{
 public class CellView : MonoBehaviour, IPoolable, IPointerDownHandler, IPointerUpHandler, IPointerMoveHandler
 {
  ...
  public void SetData(Sprite sprite, Vector3Int originPosition, Vector3Int initialPosition, float initialRotation,
   int columns, int rows, Vector3 scale, GridField gridField)
  {
   _gridField = gridField;
   _startPosition = _gridField.GetPoint(initialPosition);
  ...
  public async UniTask PlayMoveAnimation(Vector3Int indexPosition, int upFloor)
  {
   _startPosition = _gridField.GetPoint(indexPosition);
   DisableInteraction();
