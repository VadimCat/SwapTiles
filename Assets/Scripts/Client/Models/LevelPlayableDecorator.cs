...
namespace Client.Models
{
    ...
    {
    public event Action<Vector3Int> TileSelected;
    public event Action<Vector3Int> TileDeselected;
    public event Action<Vector3Int, Vector3Int> TilesSwapped;
    public event Action<Vector3Int, int> TileRotated;
    public event Action<Vector3Int> TileSet;
    ...
    private readonly HashSet<Vector3Int> _setTiles;
    private readonly Analytics _analytics;
    private readonly LevelData _levelData;
    private readonly List<Vector3Int> _selectedPositions = new(2);

    public Vector3Int Size => new(CurrentPoses.GetLength(0), CurrentPoses.GetLength(1));

    public Vector3Int FirstSelected => _selectedPositions.FirstOrDefault();
    ...
    public LevelPlayableDecorator(bool[,] cutTemplate, int rotationAngle, ILevel level)
    {
        ...
        _level = level;
        _setTiles = new HashSet<Vector3Int>(cutTemplate.GetLength(0) * cutTemplate.GetLength(1));

        CurrentPoses = new Cell[cutTemplate.GetLength(0), cutTemplate.GetLength(1)];

        List<Vector3Int> availableElements = new List<Vector3Int>(cutTemplate.GetLength(0) * cutTemplate.GetLength(1));
        ...
        for (var j = 0; j < cutTemplate.GetLength(1); j++)
        {
            if (!cutTemplate[i, j])
            {
                availableElements.Add(new Vector3Int(i, j));
            }
        }

        int[] shuffledIndexes = Shufflling.CreateShuffledArray(availableElements.Count);
        int elementsUsed = 0;

        for (var i = 0; i < cutTemplate.GetLength(0); i++)
        for (var j = 0; j < cutTemplate.GetLength(1); j++)
        {
            var current = new Vector3Int(i, j);
            if (cutTemplate[i, j])
            {
    ...
    public ClickResult ClickTile(Vector3Int tilePosition)
    {
    ...
    public bool TryGetRandomNotSelectedCell(out Vector3Int selectedTile)
    {
        ...
        List<Vector3Int> notSetTiles = new List<Vector3Int>();
        ...
        for (var x = 0; x < CurrentPoses.GetLength(0); x++)
        {
            for (var y = 0; y < CurrentPoses.GetLength(1); y++)
            {
                ...
                {
                    notSetTiles.Add(new Vector3Int(x, y));
                }
            }
        }
    ...
    private void CheckComplete()
    {
        ...
        for (var i = 0; i < CurrentPoses.GetLength(0); i++)
        {
            ...
            {
                var cellIndex = new Vector3Int(i, j);
    ...
    private void SetTile(Vector3Int cellIndex)
    {
    ...
    private void Swap(Vector3Int pos1, Vector3Int pos2)
    {
